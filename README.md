...
为长者设计的响应式静态网页，包括内嵌 AI 医生语音服务、用药提醒、家人关注和语音嘱托交互。手机浏览器全屏显示，电脑浏览器以约 480px 宽的手机界面居中显示。
...
- 不接入数据库、登录或真实电话。
- “点击进入”会在当前页面中启动内嵌 AI 健康助手，并显示“正在聆听”；语音页获得麦克风输入权限，声音输入与输出都由该页面处理。
- 再次点击会先触发内嵌语音页自身的红色结束通话按钮，然后显示两秒“通话结束”提示并恢复“点击进入”。
- 语音功能由同一 GitHub Pages 域名下的 `ai-for-elder` 页面提供，浏览器首次使用时可能需要用户允许麦克风权限。
